...
def initialize():
    ...
    logger.info(f"Received request to initialize with entry ID: {entry_id}")
    if not entry_id:
        logger.error("No entry_id provided in the request")
        return jsonify({"error": "entry_id is required"}), 400
    try:
        create_table(entry_id)
        logger.info(f"Initialized new portfolio with entry ID: {entry_id}")
        return jsonify({"message": "Database initialized"}), 200
    except Exception as e:
        logger.error(f"Error initializing database for entry ID {entry_id}: {e}")
...
def create_transaction():
    data = request.json
    logger.info(f"Received request to create transaction: {data}")
...
def modify_transaction(id):
    data = request.json
    logger.info(f"Received request to update transaction {id}: {data}")
...
def remove_transaction(id):
    logger.info(f"Received request to delete transaction {id}")
...
def profit_loss():
    ...
    logger.info(f"Calculated profit/loss: {result}")
